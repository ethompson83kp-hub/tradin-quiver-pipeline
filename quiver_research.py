...
Pulls Quiver data, scores signals, saves quiver_signals.json as GitHub artifact.
"""
...
def process_congress(raw):
    ...
    clusters = [k for k, v in tb.items() if len(v) >= 2 and
                (max(datetime.fromisoformat(e["filed"]).date() for e in v) -
                 min(datetime.fromisoformat(e["filed"]).date() for e in v)).days <= 7]
    for b in buys:
